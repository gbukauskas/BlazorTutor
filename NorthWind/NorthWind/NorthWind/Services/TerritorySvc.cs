...
using NorthWind.Components.Pages;
...
namespace NorthWind.Services
{
    public class TerritorySvc : CommonSvc, IDatabaseSvc<Territory, string>, IDisposable
    {
        ...
        /*
                public async Task<TerritoryWithRegion?> GetEntityByIdAsync(string id)
                {
                    var query = await this.Ctx.Territories.LeftJoin(
                            this.Ctx.Regions,
                            trt => trt.RegionId,
                            rgn => rgn.RegionId,
                            (trt, rgn) => new
                        ...
                        .Where(x => x.Teritory.TerritoryId == id && x.Teritory.IsDeleted == 0)
                    ...
                    {
                        return new TerritoryWithRegion()
                        {
                            RegionDescription = query.Region?.RegionDescription ?? "",
                            TerritoryId = query.Teritory.TerritoryId,
                            TerritoryDescription = query.Teritory.TerritoryDescription,
                            RegionId = query.Teritory.RegionId,
                            IsDeleted = query.Teritory.IsDeleted,
                            IgnoreConcurency = false,
                            Version = query.Teritory.Version,
        ...
        */
        ...
        {
            return await (
                    from trt in this.Ctx.Territories.Include(t => t.Region)
                    where trt.TerritoryId == id && trt.IsDeleted == 0
                    select trt
                ).SingleOrDefaultAsync();
        }
        ...
        public async Task<PgResponse<TerritoryWithRegion>> GetCurrentPageAsync(StateForComponent currentState)
        {
            Debug.Assert(currentState != null && currentState.PagerState != null);

            PgResponse<TerritoryWithRegion> currentPage;
            try
            {
                ...
                TerritoryWithRegion[] allRecords;

                if (totalRecords > 0)
                {
                    allRecords = await this.Ctx.TerritoriesWithRegion.FromSqlRaw(query)
                                        .Skip((currentState.PagerState.PageNumber - 1) * currentState.PagerState.PageSize)
                                        .Take(currentState.PagerState.PageSize).ToArrayAsync();
                }
                else
                {
                    allRecords = [];
                ...
                if (!String.IsNullOrEmpty(currentState.LastInsertedId) && !Array.Exists<TerritoryWithRegion>(allRecords, x => x.TerritoryId == currentState.LastInsertedId))
                {
                    var newRecord = await this.GetEntityByIdAsync(currentState.LastInsertedId);
                    if (newRecord != null)
                    {
                        var x = new TerritoryWithRegion()
                        {
                            RegionDescription = newRecord.Region == null ? "" : newRecord.Region.RegionDescription,
                            TerritoryId = newRecord.TerritoryId,
                            TerritoryDescription = newRecord.TerritoryDescription,
                        ...
                        allRecords = [.. allRecords, x];
                    }
                }
                currentPage = new PgResponse<TerritoryWithRegion>()
                {
